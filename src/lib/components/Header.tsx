...
const Header: React.FC = () => {
	...
	const pageTitle = activeLink ? activeLink.name.charAt(0).toUpperCase() + activeLink.name.slice(1) : null;
			...
			<h1 className="font-bold text-stone-900 dark:text-stone-100 text-2xl">
				...
				{pageTitle && (
					<span className="font-light max-[580px]:block max-[580px]:text-xl">
						<span className="text-stone-400 dark:text-stone-600 max-[580px]:hidden"> - </span>
						{pageTitle}
					</span>
				)}
			</h1>
				...
				{links.map((link) => (
					<Link
						...
						className={`hover:text-stone-900 dark:hover:text-stone-100 transition-colors ${
							location.pathname === link.href ? 'text-stone-900 dark:text-stone-100' : ''
						}`}
					>
						{link.name}
					</Link>
