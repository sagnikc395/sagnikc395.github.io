...
- Link annotations: `npm run annotations` (fetches metadata for links in posts/projects into `src/annotations.yaml`; network job, output is committed, never run by the build)
...
- Link previews (gwern-style): `src/lib/components/LinkPopups.tsx` renders the hover/tap card, `src/lib/annotations.ts` resolves a link to its card contents. Off-site text comes from `src/annotations.yaml` (see `scripts/fetch-annotations.ts`); links to other posts/projects reuse the `?meta` excerpt; `#section` links are quoted from the DOM (heading ids are generated by the `markdown()` renderer). Mounted inside the article on `BlogPost` and `ProjectPage` via a `containerRef`.
